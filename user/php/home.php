...
<head>
    ...
    <style>
        :root { --primary-color: #FFB774; --primary-dark: #E89C55; --text-dark: #2F2F2F; --text-light: #666; --border-color: #e8e8e8; --bg-light: #FFF9F4; }
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .hero-slide-content.active { opacity: 1; visibility: visible; position: relative; animation: slideInContent 0.8s ease; }
        @keyframes slideInContent { from { opacity: 0; transform: translateX(-30px); } to { opacity: 1; transform: translateX(0); } }
        @keyframes fadeInUp { from { opacity: 0; transform: translateY(30px); } to { opacity: 1; transform: translateY(0); } }
        .hero-text { animation: fadeInUp 0.8s ease; }
        .hero-badge { display: inline-flex; align-items: center; gap: 8px; background: white; padding: 8px 20px; border-radius: 30px; font-size: 14px; font-weight: 600; color: var(--primary-dark); margin-bottom: 20px; box-shadow: 0 4px 15px rgba(0,0,0,0.08); }
        .hero-title { font-size: 64px; font-weight: 800; line-height: 1.1; margin-bottom: 25px; color: var(--text-dark); }
        .hero-title span { color: var(--primary-dark); position: relative; display: inline-block; }
        .hero-description { font-size: 18px; line-height: 1.7; color: var(--text-light); margin-bottom: 35px; max-width: 500px; }
        .hero-buttons { display: flex; gap: 15px; flex-wrap: wrap; }
        .btn-primary, .btn-secondary { padding: 16px 35px; border-radius: 50px; font-size: 16px; font-weight: 600; text-decoration: none; transition: all 0.3s ease; display: inline-flex; align-items: center; gap: 10px; border: none; cursor: pointer; }
        .btn-primary { background: var(--primary-color); color: white; box-shadow: 0 8px 25px rgba(255, 183, 116, 0.4); }
        .btn-primary:hover { background: var(--primary-dark); transform: translateY(-3px); box-shadow: 0 12px 35px rgba(255, 183, 116, 0.5); }
        .btn-secondary { background: white; color: var(--text-dark); box-shadow: 0 4px 15px rgba(0,0,0,0.1); }
        .btn-secondary:hover { background: var(--text-dark); color: white; transform: translateY(-3px); }
        
        .hero-image { position: relative; }
        .hero-image-slider { position: relative; width: 100%; height: 500px; border-radius: 30px; overflow: hidden; box-shadow: 0 20px 60px rgba(0,0,0,0.15); }
        .hero-slide { position: absolute; width: 100%; height: 100%; opacity: 0; transition: opacity 0.8s ease-in-out; }
        .hero-slide.active { opacity: 1; }
        .hero-slide img { width: 100%; height: 100%; object-fit: cover; }
        
        .main-slider-nav { position: absolute; top: 50%; transform: translateY(-50%); background: white; border: none; width: 55px; height: 55px; border-radius: 50%; cursor: pointer; display: flex; align-items: center; justify-content: center; font-size: 24px; color: var(--text-dark); transition: all 0.3s ease; z-index: 100; box-shadow: 0 4px 20px rgba(0,0,0,0.15); }
        .main-slider-nav:hover { background: var(--primary-color); color: white; transform: translateY(-50%) scale(1.1); box-shadow: 0 6px 30px rgba(255, 183, 116, 0.4); }
        .main-slider-prev { left: 20px; }
        .main-slider-next { right: 20px; }
        .main-slider-dots { position: absolute; bottom: 40px; left: 50%; transform: translateX(-50%); display: flex; gap: 12px; z-index: 100; }
        .main-slider-dot { width: 14px; height: 14px; border-radius: 50%; background: rgba(255,255,255,0.6); cursor: pointer; transition: all 0.3s ease; border: 2px solid white; }
        .main-slider-dot.active { background: var(--primary-color); transform: scale(1.3); box-shadow: 0 0 10px rgba(255, 183, 116, 0.6); }

        .float-element { position: absolute; animation: float 6s ease-in-out infinite; }
        .float-element img { width: 100%; height: auto; display: block; }
        .float-1 { top: 10%; left: 5%; width: 60px; animation-delay: 0s; }
        .float-2 { top: 70%; left: 10%; width: 40px; animation-delay: 1s; }
        ...
        .feature-icon { width: 80px; height: 80px; margin: 0 auto 25px; background: linear-gradient(135deg, var(--primary-color), var(--primary-dark)); border-radius: 50%; display: flex; align-items: center; justify-content: center; box-shadow: 0 8px 20px rgba(255, 183, 116, 0.3); }
        .feature-icon img { width: 40px; height: 40px; object-fit: contain; display: block; }
        ...
        .section-subtitle { font-size: 14px; font-weight: 600; color: var(--primary-dark); letter-spacing: 2px; text-transform: uppercase; margin-bottom: 12px; }
        .section-title { font-size: 42px; font-weight: 800; color: var(--text-dark); margin-bottom: 15px; }
        .section-description { font-size: 17px; color: var(--text-light); max-width: 600px; margin: 0 auto; }
        .categories-wrapper { max-width: 1300px; margin: 0 auto; position: relative; z-index: 1; }
        .categories-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 25px; }
        .category-card-new { position: relative; height: 260px; border-radius: 20px; overflow: hidden; cursor: pointer; transition: all 0.4s cubic-bezier(0.4, 0, 0.2, 1); background: white; border: 2px solid var(--border-color); display: flex; flex-direction: column; align-items: center; justify-content: center; text-align: center; padding: 30px; }
        .category-card-new:hover { transform: translateY(-12px); border-color: var(--primary-color); box-shadow: 0 20px 50px rgba(255, 183, 116, 0.25); }
        .category-icon-wrapper { width: 100px; height: 100px; background: linear-gradient(135deg, var(--bg-light) 0%, #FFF5EC 100%); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin-bottom: 20px; transition: all 0.4s ease; position: relative; }
        .category-card-new:hover .category-icon-wrapper { background: linear-gradient(135deg, var(--primary-color) 0%, var(--primary-dark) 100%); transform: scale(1.1) rotate(10deg); box-shadow: 0 15px 40px rgba(255, 183, 116, 0.4); }
        .category-icon-new img { width: 50px; height: 50px; object-fit: contain; display: block; transition: all 0.3s ease; }
        .category-card-new:hover .category-icon-new img { transform: scale(1.1); }
        .category-name-new { font-size: 20px; font-weight: 700; color: var(--text-dark); margin-bottom: 8px; transition: color 0.3s ease; }
        .category-card-new:hover .category-name-new { color: var(--primary-dark); }
        .category-desc-new { font-size: 14px; color: var(--text-light); line-height: 1.5; transition: color 0.3s ease; }
        .category-arrow { position: absolute; bottom: 20px; right: 20px; width: 35px; height: 35px; background: var(--bg-light); border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 16px; color: var(--primary-dark); opacity: 0; transform: translateX(-10px); transition: all 0.3s ease; }
        .category-card-new:hover .category-arrow { opacity: 1; transform: translateX(0); background: var(--primary-color); color: white; }
        .view-all-categories { text-align: center; margin-top: 50px; }
        ...
        .p-img-box::before { content: ''; position: absolute; top: 0; left: 0; right: 0; bottom: 0; background: linear-gradient(to bottom, transparent 60%, rgba(0,0,0,0.05)); z-index: 1; pointer-events: none; }
        ...
        .p-badge { position: absolute; top: 10px; right: 10px; background: rgba(255, 255, 255, 0.95); backdrop-filter: blur(10px); padding: 4px 10px; border-radius: 20px; font-size: 10px; font-weight: 600; color: #555; z-index: 2; box-shadow: 0 2px 8px rgba(0,0,0,0.1); }
        
        .rank-badge { position: absolute; top: 10px; left: 10px; padding: 4px 10px; border-radius: 20px; font-size: 11px; font-weight: 700; z-index: 2; box-shadow: 0 4px 10px rgba(0,0,0,0.15); display: flex; align-items: center; gap: 4px; }
        .rank-badge.rank-1 { background: linear-gradient(135deg, #FFD700, #FFA500); color: white; }
        .rank-badge.rank-2 { background: linear-gradient(135deg, #C0C0C0, #A8A8A8); color: white; }
        .rank-badge.rank-3 { background: linear-gradient(135deg, #CD7F32, #B87333); color: white; }

        .p-info { padding: 15px; flex: 1; display: flex; flex-direction: column; }
        .p-title { font-size: 15px; font-weight: 600; color: #222; margin-bottom: 8px; line-height: 1.4; text-decoration: none; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; display: block; min-height: 22px; }
        .p-title:hover { color: #FFB774; }
        .p-sold { font-size: 12px; color: #888; margin-bottom: 5px; display: flex; align-items: center; gap: 4px; }
        .p-sold i { color: #FFB774; font-size: 12px; }
        ...
        @media (max-width: 640px) { .products-grid { grid-template-columns: 1fr; } .categories-grid { grid-template-columns: 1fr; } .hero-title { font-size: 38px; } .hero-section { height: auto; min-height: 500px; padding: 60px 0; } .hero-image-slider { height: 350px; } .main-slider-nav { width: 40px; height: 40px; font-size: 16px; } .category-card-new { height: 220px; } .section-title { font-size: 28px; } }

        .cta-section { padding: 100px 40px; background: linear-gradient(135deg, var(--primary-color) 0%, var(--primary-dark) 100%); color: white; text-align: center; position: relative; overflow: hidden; }
        .cta-content { max-width: 800px; margin: 0 auto; position: relative; z-index: 2; }
        .cta-title { font-size: 48px; font-weight: 800; margin-bottom: 20px; line-height: 1.2; }
        .cta-description { font-size: 20px; margin-bottom: 35px; opacity: 0.95; }
...
<section class="hero-section">
    ...
    <div class="hero-content">
        <div class="hero-slide-content active">
            ...
            <div class="hero-image"><div class="hero-image-slider"><div class="hero-slide active"><img src="../images/dog_and_cat1.jpg" alt="Happy Pets"></div></div></div>
        </div>
        <div class="hero-slide-content">
            ...
            <div class="hero-image"><div class="hero-image-slider"><div class="hero-slide active"><img src="../images/images1.jpg" alt="Cute Dog"></div></div></div>
        </div>
        <div class="hero-slide-content">
            <div class="hero-text">
            ...
            <div class="hero-image"><div class="hero-image-slider"><div class="hero-slide active"><img src="../images/images2.jpg" alt="Playful Cat"></div></div></div>
        </div>
        <div class="hero-slide-content">
            <div class="hero-text">
            ...
            <div class="hero-image"><div class="hero-image-slider"><div class="hero-slide active"><img src="../images/images3.jpg" alt="Happy Puppy"></div></div></div>
        </div>
    </div>
...
<section class="features-section">
    <div class="features-container">
        <div class="feature-card"><div class="feature-icon"><img src="../images/fast-delivery.png" alt="Shipping"></div><h3 class="feature-title">Free Shipping</h3><p class="feature-desc">Free delivery on orders over RM 50.</p></div>
        <div class="feature-card"><div class="feature-icon"><img src="../images/diamond.png" alt="Quality"></div><h3 class="feature-title">Premium Quality</h3><p class="feature-desc">Only the finest products.</p></div>
        <div class="feature-card"><div class="feature-icon"><img src="../images/padlock.png" alt="Secure"></div><h3 class="feature-title">Secure Payment</h3><p class="feature-desc">Safe and encrypted transactions.</p></div>
        <div class="feature-card"><div class="feature-icon"><img src="../images/support.png" alt="Support"></div><h3 class="feature-title">24/7 Support</h3><p class="feature-desc">Expert pet care advice.</p></div>
    </div>
</section>
...
let mainAutoSlideInterval = setInterval(() => { changeMainSlide(1); }, 6000);
...
function resetMainAutoSlide() {
    ...
    mainAutoSlideInterval = setInterval(() => { changeMainSlide(1); }, 6000);
}
const heroSection = document.querySelector('.hero-section');
if (heroSection) {
    heroSection.addEventListener('mouseenter', () => { clearInterval(mainAutoSlideInterval); });
    ...
    $.ajax({
        ...
        success: function(res) {
            if (res.status === 'login_required') {
                if(confirm("Please login to save items.")) {
                    window.location.href='login.php';
                }
